...
class moving_image {
  private: 
    ...
    enum TipoMovimiento {NINGUNO, IZQUIERDA, DERECHA, ARRIBA, ABAJO, ROTAR, UNDO, REDO};
    TipoMovimiento ultimoTipo = NINGUNO;
    int ultimaDistancia = 0;

    // Estructura para guardar las caracteristicas necesarias para recrear cada movimiento
    struct PasoHistorial {
      TipoMovimiento tipo;
      int distancia;
    };

    // Vector para guardar el historial de movimientos.
    std::vector<PasoHistorial> historialGlobal;

    // Contador para los nombres de archivos en repeat_all
    int frameCounter = 0; 
  ...
  public:
    ...
    red_layer[i][j] = tmp_layer[i][j];
      ...
      ultimaDistancia = d; 
      historialGlobal.push_back({IZQUIERDA, d});
    }
    ...
    red_layer[i][j] = tmp_layer[i][j];
      ...
      ultimoTipo = DERECHA; 
      ultimaDistancia = d; 
      historialGlobal.push_back({DERECHA, d});
    }

    // Función que similar desplazar la imagen, de manera circular, d pixeles a arriba
    void move_up(int d) {
      //guardamos el estado actual de la imagen en el stack
      guardarEstadoImagen();
      unsigned char **tmp_layer = new unsigned char*[H_IMG];
    ...
    red_layer[i][j] = tmp_layer[i][j];
      ...
      ultimoTipo = ARRIBA; 
      ultimaDistancia = d; 
      historialGlobal.push_back({ARRIBA, d});
    }
    ...
    red_layer[i][j] = tmp_layer[i][j];
      ...
      ultimoTipo = ABAJO; 
      ultimaDistancia = d; 
      historialGlobal.push_back({ABAJO, d});
    }

    // Función rotar la imagen en 90 grados en sentido antihorario
    void rotate() {
      //se guarda el estado actual de la imagen en el stack
      guardarEstadoImagen();
      unsigned char **tmp_layer = new unsigned char*[H_IMG];
      for(int i=0; i < H_IMG; i++) 
        tmp_layer[i] = new unsigned char[W_IMG];
      ...
      delete[] tmp_layer;
      ultimoTipo = ROTAR;
      historialGlobal.push_back({ROTAR, 0});
    }
  ...
  //Metodo que devuelve el estado de la imagen anterior al ultimo movimiento
    void undo(){
      ...
      historialGlobal.push_back({UNDO, 0}); 
    }

    // Metodo que cambia el estado actual de la imagen aplicando el ultimo movimiento revertido con redo()
    void redo(){
      ...
      for(int i=0; i < H_IMG; i++)
        ...
        historialGlobal.push_back({REDO, 0}); 
    }

  // Metodo que cambia el estado actual de la imagen aplicando el ultimo movimiento realizado
  void repeat() {
  ...
  // Metodo que repite toda la secuencia de movimientos numerada realizada sobre la imagen desde su creacion
    void repeat_all() {
      // Se resetea la imagen al estado inicial
      for(int i=0; i < H_IMG; i++)
          for(int j=0; j < W_IMG; j++) {
              red_layer[i][j] = DEFAULT_R;
              green_layer[i][j] = DEFAULT_G;
      ...
      // Se guarda el estado inicial como primer frame
      char nombre[50];
      sprintf(nombre, "pelicula_frame_%03d.png", frameCounter++);
      draw(nombre);

      // Se ejecuta el historial
      // Se usa una flag temporal para que la ejecucion del historial no se grabe a si misma
      std::vector<PasoHistorial> copiaHistorial = historialGlobal;
      historialGlobal.clear(); // Esto para evitar recursion infinita

      for (const auto& paso : copiaHistorial) {
          switch (paso.tipo) {
              case IZQUIERDA: move_left(paso.distancia); break;
              case DERECHA:   move_right(paso.distancia); break;
              case ARRIBA:    move_up(paso.distancia); break;
              case ABAJO:     move_down(paso.distancia); break;
              case ROTAR:     rotate(); break;
      ...
      // Se restaura el historial por si se quiere llamar a repeat_all() otra vez
      historialGlobal = copiaHistorial;
    }
